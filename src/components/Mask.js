...
const Mask = ({
  ...
  const translatedEyePositionX = eyePosition => {
    return eyePosition.x - eyeWidth / 2 - containerX;
  };

  const translatedEyePositionY = eyePosition => {
    return eyePosition.y - eyeWidth / 2 - containerY;
  };
  ...
  const translatedNosePositionX = noseBasePosition => {
    console.log('111111111111', noseBasePosition);
    return noseBasePosition.x - noseWidth / 2 - containerX;
  };

  const translatedNosePositionY = noseBasePosition => {
    return noseBasePosition.y - noseWidth / 2 - containerY;
  };
  ...
  const noseStyle = () => ({
    fontSize: noseWidth,
    position: 'absolute',
    left: translatedNosePositionX(noseBasePosition),
    top: translatedNosePositionY(noseBasePosition),
